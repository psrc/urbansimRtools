...
Input data are csv files exported from the test run 62. Note that only data in the baseyear and development project proposals created in the first year are needed, which would be the same for any run. Thus, the method is not dependent on simulation results. The only run-dependent factor is how the re-development filter is set. 
